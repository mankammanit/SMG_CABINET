...
char *readtft(){
uint8_t length;
static char message_tft[64];
memset(message_tft, 0, sizeof(message_tft));
const int TFT_BUF_SIZE = 64;
...
char *readec(){
static char message_ec[5];
memset(message_ec, 0, sizeof(message_ec));
const int EC_BUF_SIZE = 5;
esp_log_level_set("RX_TASK1", ESP_LOG_INFO);
uint8_t* ec_data = (uint8_t*) malloc(EC_BUF_SIZE+1);
const int ec_rx = uart_read_bytes(MY_UART1, ec_data, EC_BUF_SIZE, 100 / portTICK_RATE_MS);
if (ec_rx > 0) {
ec_data[ec_rx] = 0;
...
void init_tft()
{
  Configure_Uart(MY_UART2, TXD2_PIN, RXD2_PIN, 115200);
